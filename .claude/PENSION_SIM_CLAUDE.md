...
```javascript
{
  ...
  isa: {
    ...
    transferStartYM: '2027-05',    // 최초 이체 가능 시점 (RIA 의무보유 해제 이후)
    transferRepeatMonth: 1         // 이후 매년 이 월에 반복 이체 시도 (RIA 잔액 0 될 때까지, 금액은 자동 계산)
  },
  retire: {
    ym: '2028-12',
...
이체 시점 계산 (반복 스케줄, 고정 이벤트 아님 — 2026-07-10 정정):
  최초 이체(transferStartYM) 이후, 매년 transferRepeatMonth월마다 RIA 잔액이
  0이 될 때까지 이체를 계속 시도한다. RIA는 연 복리성장하므로 이체 횟수는
  고정되지 않으며, 잔액이 남아있는 한 매년 반복된다.

  가용액 = (가입 연차 × 2,000만) - VOO납입누계 - 이전이체누계
  이체액 = min(가용액, RIA잔액)
  초과분은 RIA에 잔류 → 다음 이체 시점(내년 transferRepeatMonth월)으로 이월
  RIA 잔액이 0이 되면 그 시점부터 이체 자동 중단
  ISA 만기(isaConversion.maturityYM) 시점에 RIA 잔액이 남아있으면 자동 재가입/가속
  이체 없이 경고만 표시 (재가입 여부는 별도 정책 결정 사항)

RIA 의무보유:
  2026.04까지 단계적 매수 완료
  일괄 해제 시점: 2027.05
  최초 이체 최소 시점(transferStartYM): 2027.05 이후
```
