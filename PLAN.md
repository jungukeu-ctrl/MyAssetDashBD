...
- [ ] (2026-07-09) IRP2 실제수령개시 나이 기본값 70세→65세로 변경(`ps-config.js` `irp2.withdrawalStartAge`). `.claude/PENSION_WITHDRAWAL.md` §2 단계별 인출 구조 표(70세~ IRP 연금 개시 서술)는 옛 70세 기준 narrative라 이번 변경으로 사실과 어긋남 — 전략 세션에서 IRP2 실수령 시점을 65세로 확정할지, 표를 갱신할지 논의 필요.
...
- [ ] (2026-07-09) O-DRIP-CONDITIONAL 구현·검증 중 발견한 재무적 사실 — 버그 아니라 현재 파라미터 조합의 특성: IRP1은 2051년경 소진되지만 **IRP2는 퇴직금(1.4억원)+연 9% 성장률이 §9-2 인출목표(갭필링)보다 훨씬 빨라 시뮬레이션 종료(2074-12, 만 100세)까지 전혀 소진되지 않음**. 이 때문에 이번에 구현한 "IRP1·IRP2 둘 다 소진 시 O(리얼티인컴) DRIP→현금인출 전환" 로직이 기본 파라미터로는 시뮬레이션 전 기간에 걸쳐 한 번도 발동하지 않음(코드 경로 자체는 스트레스 시나리오로 별도 검증 완료, 정상 동작). O 배당을 평생 재투자만 하고 생활비로 쓸 일이 없는 게 실제 의도인지, 아니면 `rates.IRP2`(현 9%) 하향·`withdrawal.irp2MonthlyTarget`(현 150만원) 상향 등으로 IRP2가 실제로 소진되는 시나리오를 원하는지 전략 세션에서 논의 필요.
- [ ] (2026-08-07) RIA 양도소득세 계산 기준 두 차례 정정(RIA-CGT-CALC→RIA-CGT-REVERT) — ①최초 "매도대금(saleAmount) 기준" 확정 → ②사용자가 첨부한 구글시트(세무전문가 컨설팅+키움양도세조회 확정치)를 웹 검증한 결과 "**양도차익(realizedGain) 기준**"이 맞는 것으로 최종 정정. 확정 계산식: RIA최종공제금액=양도차익×최종공제율 → 과세대상(공제전)=양도차익−RIA최종공제금액 → 과세표준=max(0, 과세대상(공제전)−250만원) → 예상세액=과세표준×22%. `realizedGain`=18,196,392원(SPY 51주 17,848,770원+VOO 347,622원, 키움양도세조회 완료). 공식 RIA 상품설명서 PDF(신한증권 등)는 방화벽 정책(EGRESS_BLOCKED)으로 직접 열람 불가해 구글시트 대조로만 검증함 — 전략 세션에서 공식 상품설명서 원문과 최종 대조 강력 권장.

## 🧠 전략 세션 최근 결정
> Claude.ai 전략 세션이 업데이트. Claude Code는 세션 시작 시 이 섹션을 반드시 확인.
> 결정이 코드에 영향을 미칠 경우 해당 파라미터/로직 변경 여부를 먼저 검토.

- (2026-09-21) 이 프로젝트의 관리 허브를 Claude.ai 프로젝트로 확정.
  HISTORY.md(의사결정 로그)·index.md(자원 지도) 신규 생성.
  Claude Code ↔ Claude.ai 간 브리지: 이 섹션(🧠)과 🔔 섹션을 통해 양방향 핸드오프.
- (2026-08-07) 연금계좌 매수 무조건 재개. 조건(예상수익률 2.24%) 폐지.
  연금저축 100만/월, IRP 25만/월. (ps-config.js `pension.baseMonthly=1000000` 기준 유지)
- (2026-07-08) Claude Code 위임 원칙 확정.
  Claude.ai는 코드 직접 수정 안 함 → 코드작업지시서 작성 후 Claude Code에 위임.
...
| RIA-CGT-CALC | RIA 세제혜택 조정 공제율 카드에 예상 양도소득세 계산 추가 — `PS_RIA_TAX_BENEFIT`에 `basicDeduction`(250만원)·`capitalGainsTaxRate`(22%) 필드 추가. `ps-engine.js`에 `calcRiaCapitalGainsTax()` 신규(기존 `calcRiaAdjustedDeduction` 재사용, 공개 API 등록) — RIA 특례는 매도차익이 아닌 **매도대금**(saleAmount)에 최종공제율을 적용하는 구조로 확인되어(사용자 정정), 환산RIA혜택금액=매도대금×최종공제율 → 과세대상(공제전)=매도대금−환산혜택금액 → 과세표준=max(0, 과세대상(공제전)−250만원) → 예상세액=과세표준×22%로 계산. RIA 미이용 시 예상세액·절감액 비교값도 함께 산출. `ps-table.js` `_renderRiaCard`/`refreshRiaPreview` 2개 함수만 신규 함수 호출로 교체, DOM 요소 5개(`pension-ria-benefit`/`-taxbase`/`-tax`/`-notax`/`-saved`) 추가해 해외지수ETF 순매수액 입력 시 실시간 갱신. `ps-positive`/`ps-negative` 기존 클래스 재사용(CSS 추가 없음). 검증(node vm): 순매수 0원 시 세액 0원(전액 공제), 8월 2,500만원(가중50%) 매수 가정 시 과세표준 정확히 1,000만원(=가중순매수액−250만원)·세액 220만원, 실측 1~3월 순매수(3,298,047원) 반영 시 세액 175,570원으로 계산 정상 확인. 기존 `calcRiaAdjustedDeduction` 반환값 회귀 없음(수정 전후 동일 출력 확인) | `js/pension/ps-config.js`, `js/pension/ps-engine.js`, `js/pension/ps-table.js` | 2026-08-07 |
| RIA-CGT-YM-RELOAD-FIX | "대상 월" 변경 시 납입 실적 폼 필드(연금저축/IRP1/IRP2/해외지수ETF 순매수액/메모)가 자동 갱신되지 않던 버그 수정 — 기존엔 "선택 월 불러오기" 버튼을 눌러야만 필드가 채워져, 버튼 없이 월만 바꾸면 RIA 카드 실시간 미리보기가 이전 달 값을 현재 선택 월 값으로 오인해 계산하던 문제(사용자가 2월 선택 후 확인 과정에서 발견). `ps-table.js` `_bind()`에 `_fillFormFromYm(ym)` 신규 함수 추가, `ymEl` `'input'` 리스너 중 가장 먼저 등록해 월 변경 즉시 폼을 해당 월 저장값으로 채운 뒤 `refreshStatus`/`refreshRiaPreview`가 최신 값으로 재계산되도록 함. 기존 "선택 월 불러오기" 버튼·표의 월 링크 클릭 핸들러는 무변경(중복 로직 있으나 범위 밖) | `js/pension/ps-table.js` | 2026-08-07 |
| RIA-CGT-REVERT | RIA 양도소득세 계산 기준을 매도대금→양도차익으로 원복(RIA-CGT-CALC 정정) — 사용자가 첨부한 구글시트(세무 전문가 컨설팅 기반 계산기, "RIA 양도차익(수익금) 17,848,770원"·"RIA 매도 금액 49,511,610원" 키움양도세조회 확정치 포함)를 웹 검증한 결과, 세액은 매도대금이 아닌 양도차익에 최종공제율을 적용해야 함을 확인(매도대금 5,000만원 한도는 RIA 혜택 적용 자격 한도일 뿐, 세액 계산식의 기준값이 아님). `ps-config.js`: `PS_RIA_TAX_BENEFIT`에 `realizedGain: 18196392`(SPY 51주 17,848,770원+VOO 347,622원, 키움양도세조회 완료) 추가. `ps-engine.js`: `calcRiaCapitalGainsTax()` 세액 계산부를 `saleAmount` 대신 `realizedGain` 기준으로 수정(공제율 조정비율의 분모는 원래대로 `saleAmount` 유지, 이 부분은 처음부터 맞았음), 반환값에 `gain` 필드 추가. `ps-table.js`: `_renderRiaCard`/`refreshRiaPreview`에 "매도차익(양도소득)" 표시 줄 추가(`pension-ria-gain`). 검증(node vm): 순매수 0원 시 세액 0원(전액 공제), 구글시트 예시 입력(가중 순매수 9,219,575원) 재현 시 공제율 81.37896%로 시트 원문과 소수점까지 일치 확인. 공식 RIA 상품설명서 PDF는 egress 정책으로 직접 열람 불가해 구글시트 대조로만 검증 — 전략 세션 재확인 필요(상단 체크리스트 참조) | `js/pension/ps-config.js`, `js/pension/ps-engine.js`, `js/pension/ps-table.js` | 2026-08-07 |
| RIA-CGT-FORMULA-DISPLAY | RIA 카드의 "RIA 최종 공제금액"/"과세표준"/"예상 양도소득세" 3줄이 최종 숫자만 보여줘 계산 과정을 알기 어렵다는 피드백 반영 — "입력값 연산 = 결과값" 형태(예: "1,820만 × 91.9% = 1,671만")로 변경, 과세표준이 음수라 0원으로 처리될 때는 그 이유("음수는 0원 처리")도 표시. `ps-table.js`에 `_riaTaxLines(calc)` 공용 함수 신규 추가, `_renderRiaCard`(초기 렌더)·`refreshRiaPreview`(실시간 미리보기) 둘 다 이 함수를 공유해 두 경로 표시가 항상 일치하도록 함. 계산 로직(`ps-engine.js`)은 무변경, 순수 표시 형식만 개선 | `js/pension/ps-table.js` | 2026-08-07 |
...
| PS-INFLATION-INDEXING | 인출/수령 금액 물가연동(실질가치 고정) 적용 — 기존 모든 인출/수령 금액이 명목 고정값이던 것을 물가상승률만큼 매년 복리 증액하도록 변경. 작업지시서가 지목한 `ps-withdrawal.js`의 `WD_TAX_FREE_MONTHLY` 등 상수는 `PS-DECUMULATION-INTEGRATION` Phase3(2026-07-03)에서 이미 제거되어 실존하지 않음을 확인 — `ps-withdrawal.js`는 `ps-engine.js`의 `withdrawalLog`를 그대로 읽는 표시 계층이라 별개 계산 시스템이 아님(작업지시서 3번 질문 해소, 코드 변경 불필요). `ps-config.js`: `PS_DEFAULT_PARAMS.inflation.annualRate`(기본 2.5%/년) 신규 추가. `ps-engine.js` `_stepMonth()`: `inflationMultiplier`(PS_START_YM 2026 기준 경과연수 복리) 계산 후 §7-1 연금저축 인출목표·§7-2 `IRP_MONTHLY_TARGET`/`pensionGap`·§7-3 `nationalPension.monthly`·§7-4 `overallShortfall` 4곳에 적용. `tax.separateTaxThreshold`(연1,500만 cap15m 캡)는 세법 고정값이라 제외. `ps-settings.js`: 카드1(수익률 설정)에 "물가상승률" 입력 필드 추가. 사용자 확인 필요했던 이슈: 국민연금 물가연동 기산점을 작업지시서 정의(PS_START_YM 2026 기준)와 검증예시(국민연금 개시연도 2039 기준, 424만원) 중 어느 쪽으로 할지 모순 발견 → 사용자가 전항목 2026년 기준으로 통일 확정, 검증예시값을 589만원으로 정정. 검증(node vm): ① `inflation.annualRate=0` 시 수정 전 코드(git HEAD) 대비 `plan.byAccount`/`withdrawalLog`/`calc()` 결과 JSON 완전 일치(하위호환 100%) ② 기본값(2.5%) 2074년(100세) 국민연금 월액=180만원×(1.025)^48≈588.9만원(정정된 예상치와 일치) ③ 55~100세 전 구간 NaN·음수 없음 확인 | `js/pension/ps-config.js`, `js/pension/ps-engine.js`, `js/pension/ps-settings.js`, `.claude/PENSION_WITHDRAWAL.md` | 2026-07-09 (PR #100 main 병합완료) |
| WD-TRACK-UNIFY | 인출 시뮬레이터 잔액표 ↔ 소득원(wd)의 plan/forecast 트랙 불일치 수정 — `ps-withdrawal.js` `calc()`에서 잔액(`balances`)은 계좌별 개별 fallback(`fcv>0 ? fcv : plv`)으로, wd(`withdrawalLog`)는 통째로(`fcLog || plLog`) 트랙을 골라 서로 다른 트랙이 섞이던 버그. forecast에서 IRP1·IRP2가 소진(0)돼 O DRIP이 현금인출로 전환됐는데 잔액표는 forecast의 0 대신 plan의 양수 IRP1을 표시(73세/2047-02 사례: 소득원 "O 현금인출 전환" ↔ 잔액표 IRP1 1,093만원 양수)하던 모순. **옵션A 채택**: `forecast.withdrawalLog[idx]` 존재 여부로 트랙 하나를 골라 `balances`·`wd`를 통째로 같은 트랙(`src = fcLog ? fc : pl`)에서 가져오도록 통일, 계좌별 개별 fallback 제거. git 이력 확인 결과 계좌별 fallback은 최초 구현(#71, `97a98e2`, wd가 하드코딩 타임라인이던 시절)에서 도입됐고 Phase3(`ba15db5`)에서 wd만 withdrawalLog wholesale로 바뀌며 트랙이 갈라진 것 — 원래 목적(forecast 데이터 없을 때 plan 보완)은 옵션A의 트랙 단위 선택이 그대로 계승하므로 옵션B 불필요. 표시 나이(55~100세)는 전부 2029-02 이후=`lastActualYM`(2026-06) 이후라 `fcLog` 항상 non-null → 초기 구간 회귀 없음. 검증(node vm, `PensionEngine.run()`+`PensionWithdrawal.calc()`): ① 실적 데이터 주입 65/70/73/75/80/90세 전부 잔액(IRP1·IRP2)↔`oDripActive` 정합 ② 실적 없음(순수 plan) 동일 나이 정합·회귀 없음 ③ 버그 케이스 합성 psResult(forecast IRP1=IRP2=0·`oDripActive=false`, plan IRP1=1,093만·IRP2 양수)로 `calc()` 단위 검증 → 잔액표 IRP1=IRP2=0 + "O 현금인출" 소득원 표시로 모순 해소 확인(OLD 로직이면 plan 양수 표시) | `js/pension/ps-withdrawal.js` | 2026-07-09 |
| WD-COMPREHENSIVE-TAX-FIX | 인출설정 카드 CSS 오버플로우 수정 + IRP2 기본 수령개시 나이 70→65세 + O배당 2,000만원 초과 시 금융소득종합과세(비교과세) 자동 정산 신설(§13-B, §13 excessMode와 완전 독립). ① `css/pension-sim.css`: `.ps-select` `min-width:220px` 제거(140px 고정+`max-width:100%`), `.ps-setting-row`를 grid(1fr auto, 요소가 카드 폭보다 넓으면 강제 오버플로우)→`flex; flex-wrap:wrap`으로 전환(내용이 넘칠 때만 자동 줄바꿈, 기존 한 줄 레이아웃은 유지), `.ps-card`에 `overflow:hidden` 백스톱 추가, `.ps-input`에 `max-width:100%` 추가. ② `ps-config.js`: `irp2.withdrawalStartAge` 70→65(전략 세션 반영 필요 섹션에 기록). ③ `_pensionIncomeDeduction`/`_comprehensiveIncomeTax`(기존 `ps-withdrawal.js` 전용 함수)를 `ps-config.js`의 `psPensionIncomeDeduction`/`psComprehensiveIncomeTax` 전역 헬퍼로 이동(로직 변경 없음, `ps-withdrawal.js` 기존 함수는 위임 wrapper로 유지해 호출부 무수정)해 `ps-engine.js`(별도 IIFE라 기존 함수 직접 호출 불가)에서도 재사용 가능하게 함. `ps-engine.js`에 `_markRealtyComprehensiveSettlement()` 신규(`_markRealtyAnnual` 뒤, `run()` 후처리 단계 호출 추가) — 연도별 O배당 2,000만원 초과 시 다른종합소득(국민연금 항상 포함 + 사적연금은 그 해 `excessTriggeredYear && excessMode==='comprehensive'`인 경우만 포함) 기준 비교과세(방식1: 2천만원×14%+초과분 종합과세 vs 방식2: 전액 15% 분리과세, 큰 쪽 채택) 계산 후 기납부세액(원천징수분) 차감한 추가납부액을 Y+1년 5월 log에 소급 기록(범위 밖이면 스킵+콘솔경고, 환급 케이스는 0 처리). `ps-withdrawal.js` `calc()`에 마이너스 소득원 항목("전년도 금융소득종합과세(비교과세) 정산") 표시 추가(`totalNet` 반영), 기존 "신고의무가 발생할 수 있습니다" 경고문구를 "excessMode 선택과 무관하게 자동 적용됩니다"로 정정. `.claude/PENSION_WITHDRAWAL.md`에 §13-B 신설(§13과 표로 비교, 계산 로직 상세 문서화). 검증(node vm, `PensionEngine.run()` 기본 파라미터 전체 시뮬레이션): 2056년부터 설정 발생(그 이전 연도는 O배당이 2,000만원 미만 — 회귀 없음 확인), `cap15m`/`separate16_5` 결과 완전 동일(사적연금 미포함 확인) vs `comprehensive`는 사적연금 합산으로 정산액이 유의미하게 커짐(예: 2056-05 296.4만원→2,147.5만원) 확인 — excessMode 3종 전환 시 §13-B가 사적연금 포함 여부만 정확히 달라지고 §13 자체 세액·excessMode 값은 변경 없음(독립성 확인). 시뮬레이션 종료 경계(2074/2075) 범위밖 스킵 경고 정상 동작 확인 | `css/pension-sim.css`, `js/pension/ps-config.js`, `js/pension/ps-engine.js`, `js/pension/ps-withdrawal.js`, `.claude/PENSION_WITHDRAWAL.md` | 2026-07-09 (PR #105 main 병합완료) |
| O-DRIP-CONDITIONAL | O(리얼티인컴) DRIP 조건부 전환 — 기존 `ps-withdrawal.js` `_calcODrip()`이 기준월→목표월 전체를 항상 DRIP 재투자로 가정하던 순수 표시용 오버레이(엔진 잔액과 완전 분리)였던 것을, O를 연금저축/IRP처럼 매달 상태를 추적하는 정식 항목으로 승격. `ps-engine.js`: `bal.realty={shares,monthlyDivPerShare,price}` 신규(VOO와 동일하게 `bal.해외주식`의 서브셋으로 취급 — §1에서 `priorRealtyKRW`를 nonVoo 계산에서 제외했다가 §7-5에서 이번 달 변동분만 반영), `_stepMonth()` §7-5에 매달 배당 계산→세후 재투자(Phase1, `wd.oDripActive=true` 기본값) 또는 현금인출(Phase2, `withdrawal.realty`) 조건부 분기 추가, `_wdInitial()`에 `oDripActive`/`oDripYear`/`oDripYearlyGross` 추가, 연간 배당 총액 소급집계용 `_markRealtyAnnual()` 신규(§13 `_markExcessYears`와 동일 패턴). **전환 판정 버그 발견·수정**: 최초 구현한 `overallShortfall>0` 단독 조건은 61세(인출 시작)~64세 구간에서 연금저축 §9-6 연1,500만원 캡(IRP는 아직 손도 안 댄 상태)만으로도 상시 양수가 되어 2035-10(만 61세)에 즉시 오발동함을 검증 중 발견 — `bal.IRP1<=0 && bal.IRP2<=0`(IRP 실제 소진) 조건을 추가해 수정. `ps-withdrawal.js`: `_calcODrip`/`O_DRIP_BASE_YM`/`_monthsBetween` 제거, 엔진의 `wd.realty*` 필드를 그대로 읽어 Phase1엔 O 소득원 미표시, Phase2엔 사적연금·국민연금과 합산해 `PS_COMPREHENSIVE_TAX_BRACKETS` 누진세율 근사 적용(⚠️ 근사 구현, 세무사 확인 문구 경고에 추가). `ps-withdrawal-ui.js`: 잔액 카드에 "O 재투자 중"/"O 현금인출 전환" 배지 추가. 검증(node vm): ① 회귀 테스트 — 연금저축/IRP1/IRP2/국민연금/모든 계좌 잔액이 수정 전 코드(origin/main) 대비 0건 불일치(해외주식만 O 서브셋 분리로 의도된 차이) ② 강제소진 스트레스 시나리오(IRP 수익률 0%, 목표생활비 상향)로 전환 시점에 IRP1=IRP2=0 정확히 일치, shares 전환 후 고정, 세후 배당 KRW 계산 검증(1425주×$0.5819×1,380원≈114.4만원=엔진 산출값과 일치) 확인 ③ **기본 파라미터 검증 결과**: IRP1은 2051년경 소진되나 IRP2는 퇴직금(1.4억)+연9% 성장이 인출목표보다 훨씬 빨라 시뮬레이션 종료(2074-12, 만100세)까지 소진 안 됨 → 기본값으로는 Phase2가 시뮬레이션 기간 내 발생하지 않음(코드 경로는 스트레스 시나리오로 별도 검증 완료, 위 "전략 세션 반영 필요" 참조) | `js/pension/ps-engine.js`, `js/pension/ps-withdrawal.js`, `js/pension/ps-withdrawal-ui.js`, `.claude/PENSION_WITHDRAWAL.md` | 2026-07-09 (PR #102 main 병합완료) |
| WD-CAP15M-THEN-EXPAND | `excessMode` 4번째 옵션 `cap15m_thenExpand` 신규 — IRP1·IRP2 둘 다 소진 + 목표 생활비 부족일 때만 연금저축 과세분을 §9-6 하드캡(연 1,500만) 너머로 자동 확장 인출(종합과세 적용), 그 전까지는 `cap15m`과 완전 동일 동작하는 opt-in 모드. `ps-engine.js` `_stepMonth()` §7-4b 신규(§7-4 부족분 계산 이후, §7-5 O DRIP 전환 이전 — 연금저축이 O DRIP 현금전환보다 먼저 부족분을 메움): `bal.IRP1<=0 && bal.IRP2<=0 && overallShortfall>0 && 연금저축_비과세원금<=0` 조건에서 `Math.min(부족분, 연금저축 잔액, §9-9 한도잔여)`만큼 추가 인출, `withdrawal.taxedExpansion` 필드 신규. `ps-withdrawal.js`: taxed 소스를 기본분(기존 4.4%/3.3% 세율)·확장분(신규 `_expansionComprehensiveTax()`, 종합소득세 한계세율 근사)으로 분리 표시, `isExcessYear` 판정을 `excessMode!=='cap15m'`(4번째 옵션 추가로 오발동 위험) → `separate16_5`\|`comprehensive` 명시조건으로 정정, 확장 인출 경고문 추가. `ps-settings.js` 드롭다운에 옵션 추가+§13 안내문구 조건부 갱신. **구현 중 발견·수정한 버그**: §7-1 하드캡 적용조건이 원래 `excessMode==='cap15m'`만 검사해 신규 모드가 기존 `separate16_5`/`comprehensive`용 무제한 분기를 잘못 타 IRP 소진 여부와 무관하게 처음부터 무제한 인출되던 문제 — `excessMode==='cap15m'\|\|'cap15m_thenExpand'` 양쪽 다 캡 적용하도록 수정. §13-B(O배당 비교과세)·건보료 피부양자 소득산정은 확장분 미포함(1차 구현 단순화, 기존 로직 무변경) 정책 확정. 검증(node vm, `PensionEngine.run()` 기본파라미터 전체 시뮬레이션): ① IRP 소진 전(2026~2067-06, 497개월) `cap15m` 대비 월별 인출·잔액 완전 동일(회귀 없음) ② 첫 발동(2067-06) 시 기본분 125만원 유지+확장분만 분리과세 확인, 세금 계산 예시(연1,500만 기본+연300만 확장→확장분 세금≈178,200원/년, 실효세율5.94%, 6.6%~구간과 정합) ③ 발동 이후 90개월 전부 `cap15m` 대비 부족분 "악화" 0건("개선" 90건) ④ 연금저축 잔액 음수 0건(최소 0원, 하한 정상) ⑤ 연금저축도 소진되면 기존 O DRIP 현금전환(2073-08)으로 자연스럽게 이관 확인 | `js/pension/ps-config.js`, `js/pension/ps-engine.js`, `js/pension/ps-withdrawal.js`, `js/pension/ps-settings.js`, `.claude/PENSION_WITHDRAWAL.md` | 2026-07-10 (PR #106 main 병합완료) |
| WD-CAP15M-DEFAULT-EXPAND | `withdrawal.excessMode` 기본값 `cap15m`→`cap15m_thenExpand` 변경 — IRP1·IRP2 소진 후에도 매번 드롭다운을 수동 전환해야 하던 번거로움 해소. `ps-config.js` 기본값 필드만 변경(옵션 주석 4-way로 정리), `ps-settings.js` 드롭다운은 이미 `p.withdrawal.excessMode` 값을 그대로 읽어 selected를 결정하는 구조라 하드코딩 수정 불필요 — "기본" 라벨 태그만 `cap15m_thenExpand` 옵션으로 이동. `ps-state.js`가 세션마다 `PS_DEFAULT_PARAMS`를 딥카피만 하고 localStorage/Firebase에 `excessMode`를 영속 저장하지 않아 기존 저장값과의 충돌 없음 확인. 회귀 근거: `ps-engine.js` §7-4b 트리거가 `bal.IRP1<=0 && bal.IRP2<=0`을 요구해 IRP1·IRP2 중 하나라도 잔액이 남은 구간(61~90세 등 일반 시나리오)에서는 `cap15m`과 코드 경로가 완전히 동일(§7-1 하드캡도 두 모드 모두 적용). `.claude/PENSION_WITHDRAWAL.md` §3-2-1 설명 문구도 새 기본값 기준으로 갱신 | `js/pension/ps-config.js`, `js/pension/ps-settings.js`, `.claude/PENSION_WITHDRAWAL.md` | 2026-07-10 |
| RIA-ISA-TRANSFER-REPEAT | 인출 시뮬레이션 미래 시점(97세)에 RIA·ISA 잔액이 영구 잔존하던 버그 수정 — 원인은 2026-07-10 전략 세션에서 규명(RIA 연10% 복리성장 vs `isa.transfers` 고정 3회 이벤트만 이체, 3차 이후 남은 잔액을 옮길 이벤트가 없어 영구 잔존). `ps-config.js`: `isa.transfers`(고정 3회 배열) 제거 → `transferStartYM`('2027-05')+`transferRepeatMonth`(1) 반복 스케줄로 전환. `ps-engine.js`: `_isIsaTransferMonth()` 헬퍼 신규, `_stepMonth()` §3을 `!wd.isaConverted && bal.RIA>0 && 반복월조건`일 때 매년 계속 시도하도록 변경(이체액 계산 `calcISATransfer()`는 무변경, ISA 만기 전환 후에는 자동 중단). 실제 이체 발생 이력을 `isaLimitLog`(기존 선언만 되고 미사용이던 로그)에 기록하고, 이벤트 마커(`events`)도 고정 배열 대신 이 로그에서 파생하도록 정리. ISA 만기(2029-03) 시점에도 RIA 잔액이 남아있으면 `wd.isaMaturityRiaRemaining`에 기록(자동 재가입/가속 이체 없음, 재가입 여부는 별도 정책 결정 사항). `ps-withdrawal.js`: `isaMaturityRiaRemaining>0`이면 "ISA 재가입 여부를 검토하세요" 경고 신규. `ps-settings.js`: 작업지시서 대상 파일엔 없었으나 `isa.transfers` 배열을 직접 참조하던 설정 패널 "ISA 이체 스케줄" 카드(1~3차 이체 입력 UI)가 그대로 두면 깨지므로 `transferStartYM`/`transferRepeatMonth` 입력으로 함께 교체. 검증(node vm, `PensionEngine.run()`+`PensionWithdrawal.calc()`): ① 기본 파라미터(RIA 4,951만원)에서 기존과 동일하게 3회(2027-05/2028-01/2029-01) 이체로 RIA가 정확히 0에 수렴, 55~97세 전 구간 ISA 만기 경고 없음(회귀 없음) ② RIA를 5억원으로 인위 확대한 극단 시나리오에서 반복 이체가 계속되다 ISA 만기(2029-03) 시점 RIA 잔액 58,389만원이 남아 경고가 정확히 표시되고, 만기 이후에는 이체가 자동 중단되어 RIA가 계속 성장하는 것을 확인 | `js/pension/ps-config.js`, `js/pension/ps-engine.js`, `js/pension/ps-withdrawal.js`, `js/pension/ps-settings.js`, `.claude/PENSION_SIM_CLAUDE.md` | 2026-07-10 (PR #109 main 병합완료) |
| ISA-MATURITY-VOO-REDIRECT | 인출 시뮬레이션 97세 시점 ISA 잔액 2.4억원 잔존 버그 수정 — 원인은 `_stepMonth()` §7-4(VOO 매도 3순위 "나머지 ISA" 분배, 정상/급락 분배 각 1곳)가 `wd.isaConverted` 가드 없이 실행되어, §7-6(ISA 만기 1회성 전환) 이후에도 VOO 매도금이 계속 ISA로 유입·영구 복리성장하던 것(RIA-ISA-TRANSFER-REPEAT과 같은 부류지만 별개 코드 경로). `ps-engine.js`: 두 분배 분기 모두 `wd.isaConverted===true`면 `bal.ISA += remaining` 대신 `bal.해외주식 += remaining`(VOO도 해외주식 계좌 서브셋이므로 자연스러운 귀속처)으로 대체, 누계를 `wd.isaPostMaturityVooRedirect`(`_wdInitial()` 신규 필드)에 기록해 `withdrawal` 결과 객체로 노출. `ps-withdrawal.js`: 초기 wd 객체에 동일 필드 추가, `isaPostMaturityVooRedirect>0`이면 "ISA 만기 이후 VOO 배분 XXX만원이 해외주식 계좌로 대체 귀속되었습니다. ISA 재가입 검토 필요" 경고 신규(기존 `isaMaturityRiaRemaining` 경고와 동일 패턴). ISA 만기 이전 구간 및 §13-B/§9-9/§9-10/§9-11 등 다른 로직과는 코드 경로가 겹치지 않아 무관, 계좌 간 재배치일 뿐 총자산 총합은 불변 | `js/pension/ps-engine.js`, `js/pension/ps-withdrawal.js`, `.claude/PENSION_WITHDRAWAL.md` | 2026-07-10 (PR #111 main 병합완료) |
| TAX-SETTINGS-KEY-FIX | "세율 & 건강보험료" 카드가 항상 0.0%로 표시되던 버그 수정 — `ps-settings.js`의 `render()`/`bind()`/`syncFromState()` 3곳이 2026-07-03 PS-WITHDRAWAL-BUGFIX 때 개명된 뒤 존재하지 않는 구버전 키(`tax.rate6069`/`tax.rate70`)를 여전히 참조 중이던 것(2026-07-05 PS-EXCESS-15M-UI에서 "별개 발견(미수정)"으로 기록된 항목 처리). 3개 함수의 필드를 2행(60~69세/70세~)→3행(`ps-tax-rate5569`/`ps-tax-rate7079`/`ps-tax-rate80`, 55~69세/70~79세/80세~)으로 교체해 실제 계산 키(`ps-withdrawal.js`가 참조하는 `rate5569/rate7079/rate80`)와 일치시킴. `ps-config.js` 기본값(0.055/0.044/0.033)·세금 계산 로직(`ps-withdrawal.js`)은 원래부터 정상이라 무변경 — 이번 수정은 UI 표시·입력 전용, 계산 결과값 변화 없음(원래도 3단계 세율로 정상 계산 중이었음) | `js/pension/ps-settings.js` | 2026-07-17 |
| WD-OVERSEAS-SALE | Phase2(IRP1·IRP2 소진, `wd.oDripActive===false`) 진입 이후 O(리얼티인컴) 배당만으로 목표 생활비 부족분(`overallShortfall`)을 못 메우던 문제 수정 — 해외주식 계좌(O 포함, VOO는 §4에서 별도 관리하므로 제외)에서 부족분만큼 추가 매도해 인출액에 포함하도록 `ps-engine.js` `_stepMonth()`에 §7-6 신설. 신규 파라미터 `overseasSale{costBasisRatio(취득원가율, 기본 50%), capitalGainsTaxRate(22%), annualExemption(연 250만원)}`(`ps-config.js`)로 해외주식 양도소득세(소득세법 제118조의5·제118조의7) 근사 계산·표시(`ps-withdrawal.js`, 세금은 표시 전용이라 인출액 자체는 미차감 — 기존 다른 소득원과 동일 패턴), 설정 패널 "해외주식 매도(부족분 충당)" 고급설정 카드 추가(`ps-settings.js`). **구현 중 동적 시뮬레이션(node vm, 여러 달치 상태 전이 실제 재현)으로 발견한 결함**: `bal.해외주식`은 VOO·O를 뺀 나머지가 매달 파생 재계산되는 구조(§1 `nonVoo`)라 O를 안 건드리고 총액만 직접 차감하면 다음 달 파생 계산에서 `nonVoo`가 0으로 클램프되며 차감분이 "부활"하는 결함이 있었음(1차 지시서의 "별도 보정 불필요" 가정이 틀렸음을 확인 — 정적 코드 리뷰로는 발견 안 되고 동적 시뮬레이션에서만 드러남). "기타(non-VOO/non-O) 잔액 우선 소진 → O(`bal.realty.shares`) 비례 차감" 방식으로 재설계해 우회. 검증(node vm, 실제 구현 파일 로드): 기본 파라미터 회귀 없음(NaN·음수 잔액 없음), 강제소진 스트레스 시나리오에서 매도 구간 전체 "1,000원 초과 반등 0건" 확인(수정 전 버전은 3개월째부터 반등하던 결함 재현 후 비교 완료), 잔액이 단조 감소하다 정확히 0에서 소진되고 이후 `overallShortfall` 재차 양수 유지, 취득원가율 0%/100% 경계값에서 세금 최대/0 정확히 계산, IRP1·IRP2 잔액이 남아있는 구간(65/70세)에서는 미노출(오발동 없음) 확인 | `js/pension/ps-config.js`, `js/pension/ps-engine.js`, `js/pension/ps-withdrawal.js`, `js/pension/ps-settings.js`, `.claude/PENSION_WITHDRAWAL.md` | 2026-07-17 (PR #114 main 병합완료) |
| PS-YEARSUMMARY-SAVE-SYNC | "월별 계획 vs 실적" 화면에서 납입 실적 저장 시 "저장 완료" 상태 메시지의 연간 누적액과 옆 "연간 한도 점검" 카드의 연간 누적액이 서로 다르게 표시되던 버그 수정 — 사용자가 스크린샷으로 제보(438만원 vs 393만). 원인: `ps-table.js` 저장 버튼 핸들러가 `PensionFirebase.saveContribution()` 호출 *이전*(await 이전)에 미리 계산해둔 `validation.message` 문자열을 저장 완료 후 재검증 없이 그대로 재사용(`· 저장 완료`만 append)하던 구조 — 실제 저장된 값과 상태 메시지에 표시되는 값이 어긋날 수 있었음(저장 직전 폼 입력값과 실제 저장 후 확정된 데이터가 다를 경우). `PensionFirebase.saveContribution()`이 반환하는 실제 저장 데이터(`updated`)를 기준으로 `validateContribution()`을 다시 호출해 상태 메시지를 재계산하도록 수정 — "연간 한도 점검" 카드(`_renderYearSummary()`)와 항상 동일한 소스(`updated`)를 참조하게 되어 두 수치가 항상 일치함. 한도 검증 로직(1,800만원 상한/1,500만원 경고) 자체는 무변경 | `js/pension/ps-table.js` | 2026-08-21 |
| PS-WON-INPUT-COMMA | 원 단위 금액 입력/표시에 천단위 콤마 적용 — 사용자가 "숫자를 보기 어렵다"고 요청. HTML `type="number"`는 콤마 입력을 거부해 큰 금액(퇴직금 1.4억 등) 표시가 안 되던 근본 원인. `ps-config.js`에 공용 헬퍼 `psFormatWonInput(n)`(콤마 포함 문자열 반환)·`psAttachWonInputFormatter(el)`(타이핑 중 실시간 콤마 재포맷, 커서 위치 보정 포함) 신규 추가. `ps-settings.js`: 원 단위 금액 필드 11개(목표 월 인출액·IRP 목표 월 인출액·1주당 가격·연금저축 기본납입·분리과세 기준선·피부양자 소득기준·사적연금 면제 하한·아파트 공시가격·연간 기본공제·퇴직금·국민연금 월액)를 `_numInput`→신규 `_wonInput`(type=text+inputmode=numeric)으로 전환, `bind()`에 신규 `_bindWon()`(콤마 제거 후 parseFloat 파싱) 적용, `syncFromState()`도 `psFormatWonInput()`으로 콤마 반영. 퍼센트/나이/수량/주기 등 금액이 아닌 필드는 `_numInput`(`type="number"`) 그대로 유지해 범위 밖 대상 없음. `ps-table.js`: 납입 실적 입력 4개(연금저축·IRP1·IRP2·해외지수ETF 순매수액)도 동일하게 `type="number"`→`type="text" inputmode="numeric"` 전환, `_fmtInput`/`_fmtRiaInput`에 `toLocaleString('ko-KR')` 추가, `_bind()`에서 `psAttachWonInputFormatter` 부착. 기존 저장/검증 로직(`_parseWon`, `_readFormPatch`, `validateContribution`)은 이미 콤마 제거 파싱 구조였어서 무수정. `js/*.js`·`index.html`은 CLAUDE.md 규칙상 미수정(범위 `js/pension/`으로 한정). 검증(Playwright 헤드리스, 실제 페이지 로드): 설정 패널 원 단위 필드 초기값 콤마 표시 확인(퇴직금 `140,000,000`, 목표 월 인출액 `3,050,000`), 퇴직금 필드에 `140000000` 타이핑 중 실시간 `140,000,000` 표시 확인, `change` 이벤트 후 `PensionState.params.retire.severancePay`가 콤마 없는 순수 숫자(`140000000`, number)로 정상 저장 확인, 퍼센트 필드(`ps-rate-pension`)는 `type="number"` 그대로 유지·콤마 미적용 확인, 납입 실적 입력(`pension-contrib-연금저축`)도 타이핑 중 실시간 콤마(`1,000,000`) 확인 — JS 런타임 에러(pageerror) 0건 | `js/pension/ps-config.js`, `js/pension/ps-settings.js`, `js/pension/ps-table.js` | 2026-09-18 |
