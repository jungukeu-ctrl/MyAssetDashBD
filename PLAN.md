...
| BUG-TOSS-HISTORY | 선 그래프 투자금(점선) 과거 월별 toss 잔고 미반영 수정 — kiData.tossHistory 구조 추가, 탭 구분 TSV 이력 입력 모달(헤더 버튼), updateLineChart() 전체 구간 tossHistory 반영(최신 월 state 폴백), exportMonthlyXlsx() 월별투자금 시트 toss 합산 | `index.html`, `modal.js`, `render.js`, `export.js` | 2026-03-26 |
