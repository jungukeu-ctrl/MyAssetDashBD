...
- [ ] (2026-08-07) RIA 양도소득세 계산 로직 신규 확정(RIA-CGT-CALC) — 세제혜택 산정 기준이 "RIA 계좌의 양도소득이 5,000만원까지 비과세"가 아니라 **"RIA 계좌 매도대금(saleAmount, 49,511,610원)에 최종공제율을 곱한 금액만큼 과세대상에서 빠지는"** 구조임을 사용자가 정정. 계산식: 환산RIA혜택금액=매도대금×최종공제율 → 과세대상(공제전)=매도대금−환산혜택금액 → 과세표준=max(0, 과세대상(공제전)−250만원 기본공제) → 예상 양도소득세=과세표준×22%. 매도차익(취득원가 기준 손익)은 이 계산에 쓰이지 않음 — SPY 51주 취득원가 데이터 불필요. 전략 세션에서 이 해석(매도대금 기준 vs 양도소득 기준)이 실제 RIA 상품설명서(소비자보호검토필 제26-0460호)와 일치하는지 재확인 필요.
