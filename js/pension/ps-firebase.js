...
const PensionFirebase = (() => {
  ...
  //
  // ─── 실적(actual) 총액 산출 기준 — 6개 계좌, 평가금(eval, 잔액) 기준 ──────────
  // "계획 총액/실적 총액/차이"는 아래 6개 계좌의 eval[](평가금=계좌 잔액,
  // 납입금·투자금 아님) 합계임.
  //
  //   연금저축  → eval[3]   (2026-03부터 토스모으기(개인연금저축모으기) 자동 합산됨)
  //   IRP1      → eval[7]
  //   IRP2      → eval[8]
  //   해외주식  → eval[0]   (VOO 포함, Firebase에서 VOO만 분리 불가)
  //   RIA       → eval[10]
  //   ISA       → eval[9]
  //
  // 주의: index.html 메인 대시보드의 "총 평가금액"은 오빌·연습 계좌 등을 포함한
  // 8개 전체 계좌 기준이라 이 6개 합계보다 큼(전체 자산 vs 연금 관련 자산만).
  // 두 화면의 총액이 다르게 나오는 건 정상이며 버그 아님.
